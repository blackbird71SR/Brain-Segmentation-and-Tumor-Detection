...
def write_data_to_file(training_data_files, out_file='./test2.h5', image_shape=(128, 128, 128), subject_ids=None,normalize=True, crop=True):
    n_samples = len(training_data_files)
    # n_channels = len(training_data_files[0]) - 1
    n_channels = len(training_data_files[0])
    try:
        #hdf5_file, data_storage, truth_storage, affine_storage = create_data_file(out_file, n_channels=n_channels, n_samples=n_samples, image_shape=image_shape)
        hdf5_file, data_storage, affine_storage = create_data_file(out_file, n_channels=n_channels, n_samples=n_samples, image_shape=image_shape)
    except Exception as e:
        # If something goes wrong, delete the incomplete data file
        os.remove(out_file)
        raise e
    write_image_data_to_file(training_data_files, data_storage, image_shape, n_channels=n_channels, affine_storage=affine_storage, crop=crop)
    if subject_ids:
...
x = input('Enter path of image folder:')
write_data_to_file(x)